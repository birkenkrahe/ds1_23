...
8) How do =levels= of a =factor= and =names= of a =vector= differ?
   #+begin_src R
     vec <- c(one=1,two=2,three=3) # element names are labels
...
11) What are =factor= values good for?
    ...
    #+begin_src R
      ...
      str(mt$cyl)  # cyl vector is now a factor
    #+end_src
    ...
    #+RESULTS:
*** Data frame practice (workspace) continued
*** Data frame review questions
